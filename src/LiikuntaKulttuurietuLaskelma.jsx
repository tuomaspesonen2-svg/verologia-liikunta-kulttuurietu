...
<h1 style={{ fontFamily: HEAD, fontSize: 27, fontWeight: 800, margin: "10px 0 0", lineHeight: 1.08, letterSpacing: "-.028em", color: "#fff" }}>Palkankorotus vai liikunta- ja kulttuurietu?</h1>
<p style={{ fontFamily: BODY, fontSize: 14.5, color: "rgba(255,255,255,0.72)", margin: "10px auto 0", lineHeight: 1.55, maxWidth: 540 }}>Vertailu: sama euromäärä bruttopalkassa vs. verovapaana liikunta- ja kulttuurietuna.</p>
</div>
</div>
...
<Eyebrow>Mitä etu kattaa</Eyebrow>
<div style={{ fontFamily: BODY, fontSize: 13, color: INK, lineHeight: 1.6, marginTop: 8 }}>
<strong>Liikunta:</strong> kuntosalit, ryhmäliikunta, uimahallit, urheilutapahtumat.<br />
<strong>Kulttuuri:</strong> teatteri, konsertit, museot, elokuvat, kirjastomaksut.<br />
...
<FieldLabel right={<input type="number" min={0} max={ANNUAL_BENEFIT_MAX} step={10} value={annualBenefit} onChange={(e) => handleBenefitInput(e.target.value)} style={{ ...numInput, width: 90 }} />}>
Etu / työntekijä / vuosi
</FieldLabel>
<input className="vl-range" type="range" min={0} max={ANNUAL_BENEFIT_MAX} step={10} value={annualBenefit} onChange={(e) => setAnnualBenefit(Number(e.target.value))} />
...
Verovapaa enintään {ANNUAL_BENEFIT_MAX} €/v ({fmt(ANNUAL_BENEFIT_MAX / 12)} €/kk). Yli menevä osa olisi verotettavaa palkkaa.
</div>
</div>
...
<Eyebrow>Liikunta- ja kulttuurietu ✓</Eyebrow>
<div style={{ fontSize: 12, color: MUTED, margin: "14px 0 4px" }}>Työnantaja maksaa /kk</div>
...
<AnimBar value={calc.employerCostBenefit} max={maxBar} color={GREEN} label="Liikunta- ja kulttuurietu (ei sivukuluja)" delay={0.2} />
</div>
...
<AnimBar value={calc.employeeNetBenefit} max={calc.monthlyBenefit || 1} color={GREEN} label="Liikunta- ja kulttuurietu (veroton)" delay={0.4} />
</div>
</div>
...
{fmt0(annualBenefit)} € liikunta- ja kulttuurietuna tuottaa työntekijälle <strong style={{ color: GREEN_SOFT }}>{fmt(calc.employeeGainYear)} € enemmän</strong> vuodessa kuin sama summa palkankorotuksena. Samalla työnantaja <strong style={{ color: GREEN_SOFT }}>säästää {fmt(calc.employerSavingsYear)} €</strong> sivukuluissa.
</div>
</div>
...
<div style={{ fontSize: 12, color: MUTED, marginBottom: 4 }}>Liikunta- ja kulttuurietu yhteensä</div>
